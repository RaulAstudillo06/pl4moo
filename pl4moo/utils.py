...
def evaluate_obj_and_cost_at_X(
    ...
    if objective_cost_function is not None:
        Y, costs = objective_cost_function(X)
    else:
...
def fit_model(
    ...
    if Y.ndim == 1:
        Y_copy = Y_copy.unsqueeze(-1)

    # Outcome transform
    outcome_transform = Standardize(m=Y_copy.shape[-1], batch_shape=Y_copy.shape[:-2])
...
def get_suggested_budget(
    ...
    if (
        ...
        and (previous_budget - costs[-1] > lower_bound)
    ):
        suggested_budget = previous_budget - costs[-1].item()
